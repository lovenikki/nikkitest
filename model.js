...
Clothes = function(csv) {
  ...
  return {
    ...
    tags: csv[14].split(','),
    source: csv[15].replace(/浪·/g,"").replace(/设·/g,"").replace(/图/g,"Recipe").replace(/Maiden/g," M"),
	isSuit: csv[16],
	version: csv[17],
    deps: [],
    toCsv: function() {
    ...
    getDeps: function(indent, parentDepNum) {
      var ret = '';
        for (var i in this.deps) {
		  ...
		  var c = depinfo.c;
		  var depNumAll = 1;
		  if(depinfo.sourceType != "Customization"){
          ...
          ret += indent + '[' + depinfo.sourceType + '][' + c.type.mainType + ']'
              + c.name + ((c.own || depNumAll == 0)? '' : '[Need ' + (depNumAll)  + ']')+ '\n';
...
function clotonum(type,id){
	var mainType='';
	switch(type.split('-')[0]){
		case 'Hair': mainType='1'; break;
		case 'Dress': mainType='2'; break;
		case 'Coat': mainType='3'; break;
		case 'Top': mainType='4'; break;
		case 'Bottom': mainType='5'; break;
		case 'Hosiery': mainType='6'; break;
		case 'Shoes': mainType='7'; break;
		case 'Accessory': mainType='8'; break;
		case 'Makeup': mainType='9'; break;
  ...
  validate: function(criteria,accNum){ //accNum is the number of accessories kept finally
	for (var i in repelCates){ //remove repelCates
		...
		for (var j in repelCates[i]){
			...
			if (this.cart[currCate]) {
				...
				var currSumScore = currCate.split('-')[0] == 'Accessory' ? accSumScore(this.cart[currCate], accNum?accNum:accCateNum) : this.cart[currCate].sumScore;
	...
	if (accNum) {//keep accessories base on accNum
		...
		for (var i in category){
			...
			if (currCate.split('-')[0] == 'Accessory' && this.cart[currCate]) {
...
var accCateNum = function() {
	var cnt = 0;
	for (var i in category) {
		if (category[i].split('-')[0] == "Accessory") cnt++;
	}
	for (var i in skipCategory) {
		if (skipCategory[i].split('-')[0] == "Accessory") cnt--;
	}
	return cnt;
}();
