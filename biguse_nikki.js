...
function changeFrontFilterDiv(c){
	...
	$("#front_filter_div").append($("<button>").addClass("btn btn-default front_filter_option_biguse").attr("type", "button").text("Remove All"));
	for(var co in color){
		if(co.indexOf(c) >=0){
...
function filterClotherHTMLBiguse(btn){
	...
	var str = $(btn).text();
	 for(var i = 0 ; i < clothesDivList.length; i++){
		 if($(clothesDivList[i]).find(".color_search:first").text().indexOf(str) < 0 && str != "Remove All"){
