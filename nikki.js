	...
	if (currentLevel) {
		...
		if (currentLevel.hint) {
			...
			if (currentLevel.hint[0] && currentLevel.hint[0] != '') {
				var $hintInfo = $("<font>").text("Tips:  ").addClass("hintInfo");
				$hint.append($hintInfo).append(currentLevel.hint[0]);
			}
			if (currentLevel.hint[1] && currentLevel.hint[1] != '') {
				var $notF = $("<font>").text("Wearable Clothes:  ").addClass("not_f");
		...
		if (currentLevel.skills) {
			...
			if (currentLevel.skills[0]) {
				$shaonv = $("<font>").text("NPC's Skills(Maiden):  ").addClass("shaonvSkill");
				shaonvSkill = "";
			...
			if (currentLevel.skills[1]) {
				$gongzhu = $("<font>").text("NPC's Skills(Princess):  ").addClass("gongzhuSkill");
				gongzhuSkill = "";
			...
			if (currentLevel.skills[2]) {
				$normal = $("<font>").text("NPC's Skill:  ").addClass("normalSkill");
				normalSkill = "";
...
function filterTopAccessories(filters) {
	filters['own'] = true;
	var accCate = CATEGORY_HIERARCHY['Accessory'];
...
function filtering(criteria, filters) {
	...
	if ($("#showmore").attr("isshowmore") == 1) {
		var size = 10;
		if (result[0] && result[0].type.mainType == "Accessory")
			size = 5;
		var tsize = size;
		for (var i in result) {
...
function drawFilter() {//refactor me
	...
	for (var c in CATEGORY_HIERARCHY) {
		...
		if (CATEGORY_HIERARCHY[c].length > 1) {
			...
			out += "<label><input type='checkbox' id='all-" + c + "' onClick='toggleAll(\"" + c + "\")' checked>Select All</label><br/>";
			// draw sub categories
			for (var i in CATEGORY_HIERARCHY[c]) {
...
function doImport() {
	...
	if(names.length > 50){
		names = names.substring(0,50) + "...etc" + updating.length + "clothes";
	}
...
function searchResult(){
	...
	if (searchTxt){
		...
		if (outSet.length>0) {
			...
			$('#searchResultList').append(button_search('Suit：','searchCate'));
...
function autogenLimit(){
	...
	for(var i in clothes){
		clothes[i].calc(criteria);
		var sum_score=(clothes[i].type.mainType=='Accessory') ? Math.round(accSumScore(clothes[i],(uiFilter["acc9"]?9:accCateNum))) : clothes[i].sumScore;
...
function initEvent() {
	...
	$(".showmore").click(function(){
		...
		if($(obj[1]).attr("isshowmore") == "1"){
			$(obj[0]).text("↓ Show All Clothes ↓");
			$(obj[1]).text("↓ Show All Clothes ↓");
		}
		else{
			$(obj[0]).text("↑ Fold Wardrobe ↑");
			$(obj[1]).text("↑ Fold Wardrobe ↑");
		}
		onChangeUiFilter();
		menuFixed("clothes");
		return false;
	...
	$("#searchResultMode").click(function(){
		if ($(this).hasClass("active")) {$(this).removeClass("active");$(this).html('→Wardrobe');}
		else {$(this).addClass("active");$(this).html('→Cart');}
	});
